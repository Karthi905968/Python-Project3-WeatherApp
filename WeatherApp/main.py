	...
	def get_weather(self, event=None):
		...
		if len(city) > 2:
			...
			try:
				...
				messagebox.showerror('WeatherGuard', 'No such city in database')
			except:
				messagebox.showerror('WeatherGuard', 'No internet Connection')
				
			self.entry.delete(0, tk.END)

if __name__ == '__main__':
	root = tk.Tk()
	root.title('WeatherGuard')

	if not api_key:
		root.withdraw()
		messagebox.showerror('WeatherGuard', 'OpenWeatherMap Api Key is required\n to use this App')
		sys.exit(0)

	bg_list = [f'wallpapers/bg{i}.jpg' for i in range(1,7)]
